...
class KNNBoundaryPlotter(object):
    ...
    def knn_mesh_fitter(self):
        ...
        # get the minimum and maximum values for each of the predictor
        # variables
        ...
        # I add a buffer here (1/15th of the range) so no points are on the
        # axes
        ...
        for nn in self.nn_range:
            # fit a knn on the data with the nearest neighbors number passed
            # into the function
            knn_mod_euc = KNeighborsClassifier(
                n_neighbors=nn, weights='uniform')
            knn_mod_euc.fit(self.X, self.y)

            knn_mod_w = KNeighborsClassifier(
                n_neighbors=nn, weights='distance')
            knn_mod_w.fit(self.X, self.y)

            # Predict using the knn model on all the meshgrid points. This will let us see
            # the knn boundary of where it predicts between one class and
            # another!
    ...
    def knn_boundary_plotter(self, nn, weights):
        ...
        plt.scatter(self.v1_points, self.v2_points,
                    c=point_colors, s=self.dotsize)
